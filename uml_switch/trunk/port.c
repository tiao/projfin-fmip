...
static void send_dst(struct port *port, struct packet *packet, int len, int hub) {
...
static void handle_data(int fd, int hub, struct packet *packet, int len,
	...
	for (p = head; p != NULL; p = p->next) {
		if ((*matcher)(p->control, fd, p->data, p->data_len, data))
			break;
	}
