...
#include <errno.h>
#include <stdlib.h>
#include <signal.h>  // ISO C99 Standard: 7.14 Signal handling
...
#include <sys/un.h>
#include <sys/poll.h>    // Compatibility definitions for System V `poll' interface
#include <sys/time.h>
#include <unistd.h>
#include "switch.h"
#include "port.h"
...
static void new_port(int fd, int data_fd) {
	...
	if (req.v1.magic == SWITCH_MAGIC) {
		...
		if (req.v3.version == 3)
			...
			new_port_v1_v3(fd, req.v1.type, &req.v1.u.new_control.name, data_fd);
	} else
		new_port_v0(fd, &req.v0, data_fd);
}
...
void bind_sockets_v0(int ctl_fd, const char *ctl_name, int data_fd,
	...
	if (ctl_present && ctl_used) {
		...
		fprintf(stderr,
				"The control socket '%s' exists, isn't used, but couldn't "
					"be removed\n", ctl_name);
	if (data_present && data_used) {
		...
		fprintf(stderr,
				"The data socket '%s' exists, isn't used, but couldn't "
					"be removed\n", data_name);
	if (try_remove_ctl || try_remove_data) {
		...
		fprintf(stderr,
				"You should rerun with different, unused filenames for "
					"sockets:\n");
		fprintf(stderr, "\t%s -unix <control> <data>\n", prog);
		fprintf(stderr, "\tand run the UMLs with "
			"'eth0=daemon,,unix,<control>,<data>'\n");
...
int execute_line (char *line) {
  ...
  if (!command)
    {
      printf ("%s: No such command for uml_switch.\n", word);
      return (-1);
    }
...
int main(int argc, char **argv) {
	...
	if (compat_v0)
		...
		printf("%s attached to unix socket '%s'", prog, ctl_socket);
	...
	while (1) {
		...
		for (i = 0; i < nfds; i++) {
			...
			if (fds[i].fd == 0) {
				...
				if (new)
					new_port(fds[i].fd, data_fd);
				else
					close_descriptor(fds[i].fd);
			}
		}
	}
	out: cleanup();
